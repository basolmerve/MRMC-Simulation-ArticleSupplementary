boott2 <- function (x, theta, ..., sdfun = sdfunboot, nbootsd = 25, nboott = 200, 
          VS = FALSE, v.nbootg = 100, v.nbootsd = 25, v.nboott = 200, 
          perc = c(0.001, 0.01, 0.025, 0.05, 0.1, 0.5, 0.9, 0.95, 0.975, 0.99, 0.999)){
  call <- match.call()
  sdfunboot <- function(x, nboot, theta, ...) {
    n <- length(x)
    junk <- matrix(sample(x, size = n * nboot, replace = TRUE), 
                   nrow = nboot)
  ...
  if (!VS) {
    xstar <- matrix(sample(x, n * nboott, replace = TRUE), 
                    nrow = nboott)
  }
  else {
    xstar <- matrix(sample(x, n * v.nboott, replace = TRUE), 
                    nrow = v.nboott)
  }
  ...
  return(list(confpoints = ans1, theta = thetastar0, g = g, call = call))
}
